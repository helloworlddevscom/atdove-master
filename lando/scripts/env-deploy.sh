...
# database updates, reverting features and cache clear is handled automatically
...
then
  ...
  then
    echo
    echo -e "${ORANGE}Are you sure you want to deploy to and run database updates, revert features and clear caches on live/prod? This will wipe out any config changes that are currently on live/prod. (y/n):${BLUE}"
    echo
    read REPLY_CONFIRM
    if [[ ! "$REPLY_CONFIRM" =~ ^[Yy]$ ]]
    then
  ...
  echo
  terminus env:deploy atdove-d7."$REPLY_ENV" --note "$DEPLOY_MESSAGE"

  echo
  echo -e "${YELLOW}The Quicksilver script should have already ran database updates, reverted features, and cleared caches after the deploy,${NORMAL}"
  ...
  echo
  echo -e "${YELLOW}Running reverting features...${NORMAL}"
  echo
